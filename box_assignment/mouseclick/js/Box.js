function Box() {
	...
	this.initializeValue = function(posx,posy,dirx,diry,boxWidth,boxHeight,slope,velocity,identity){
		that.posx = posx;
		that.posy = posy;
		that.dirx = dirx;
		that.diry = diry;
		...
		that.identity = identity; //helpful in remaining session;
	} 

   this.moveBox1 = function(){
   	var xnew = this.posx + this.dirx*this.velocity;
		var ynew = this.posy + this.diry*this.slope*this.velocity;

     //console.log('the value of velocity is identity',this.identity, '', this.velocity);
